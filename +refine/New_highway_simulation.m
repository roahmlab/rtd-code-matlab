...
for j = 1:100
    ...
    frs = refine.planner.Refine_Planner(trajOptProps,worldState,HLP,Agent);
    AgentHelper1 = frs.Nhah; % takes care of online planning
    Simulator1 = rlsimulator(AgentHelper1,World,'plot_sim_flag',plot_sim_flag,'plot_AH_flag',plot_AH_flag,'save_result',save_result);
    AgentHelper1.S = Simulator1;
    Simulator1.eval = 1; %turn on evaluation so summary will be saved

    rng(j+1);
    IsDone4 = 0;
    Simulator1.epscur = j;
    Simulator1.reset();%ORIGINAL
    for i = 1:40
        AgentHelper1.planned_path = [linspace(0,1000);repmat([0;0],1,100)];
        [~,~,IsDone,LoggedSignal]=Simulator1.step([rand*2-1;rand*2-1]);
