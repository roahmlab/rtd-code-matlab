...
%         t_move
    ...
    %% methods
    methods
        function AH = NewhighwayAgentHelper(A,frs, HLP,worldState,trajOptProps,refineplanner,varargin)
            ...
            AH.refineplannerInstance = refineplanner;
            if strcmp(frs.manu_type,'speed_change')
                AH.zono_full = frs.reachableSets;
            elseif  strcmp(frs.manu_type,'lane_change')
                AH.zono_full = frs.reachableSets;
            else
        ...
        function [K,tout] = gen_parameter_standalone(AH, worldinfo,agent_state,waypoints)

            AH.waypoint = waypoints;
            AH.agent_state = agent_state;
            %The refine planner kout and tout are the trajectory and timeout 
            [traj,info] = AH.refineplannerInstance.planTrajectory(agent_state(1:6),worldinfo,waypoints);%initially it was AH.robotstate,worldinfo,AH.worldState
            K = traj;
            tout = info;
...
% 
% %             x_des_mex = x_des;
% % %             disp(worldinfo.dyn_obstacles)
% % %             disp(class(worldinfo))
% %             dyn_obs_mex = get_obs_mex(worldinfo.dyn_obstacles, worldinfo.bounds);
...
%             
% %things i need for the below code to work
% %type_text = 1,2,3
% %figure out what M is-->M_mega has idxu0. 
% %idxu0 
% 
%                 %changes
%                 type_text = type_manu;
% %                 disp(AH.zono_full)
% %                 disp(length(AH.zono_full))
% %                 disp(class(AH.zono_full))
%                 M = AH.zono_full;%the frs
% 
% 
%                 if type_manu == 1
%                     k = K(1);
...
%                 end
% %slice_pt = [8;9;10] slice_val = [5;6;7]
%                 zono_one =zonotope_slice(FRS{1}, [8;9;10], [agent_state(5);agent_state(6);agent_state(7)]);
