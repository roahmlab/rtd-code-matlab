classdef uarmtd_planner_wrapper < robot_arm_generic_planner
...
%         t_plan = 0.5; % already defined in superclass planner.m
    ...
    methods
        function P = uarmtd_planner_wrapper(varargin)
            for i = 1:nargin
...
%             P.t_plan = 0.5;
            ...
            % WRAP THE OTHER PLANNER
            trajOptProps = rtd.planner.trajopt.TrajOptProps;
            P.planner = armour.ArmourCudaPlanner( ...
                trajOptProps, armour.ArmourAgent.empty(), ...
                verboseLevel='DEBUG');
        end
...
%                             fprintf(cuda_input_file, '%.1rtd.planner.trajopt.TrajOptProps0f ', temp(ind));
%                         end
%                         fprintf(cuda_input_file, '\n');
%                     end
%     
...
%                     terminal_output = system('./../kinova_simulator_interfaces/kinova_planner_realtime/rtd_force_main_v2'); % rtd-force path
                    ...
                    % constructing robotState
                    state = [q_0(:); q_dot_0(:); q_ddot_0(:);];
                    indices = 1:length(q_0);
                    robotState = rtd.entity.states.ArmRobotState(indices, length(q_0) + indices, length(q_0) * 2 + indices);
                    robotState.state = state;

                    worldState.obstacles = [];
                    if ~isempty(world_info.obstacles)
                        worldState.obstacles = cellfun(@(obs)obs.zono, world_info.obstacles);
                    end

                    [trajectory, info] = P.planner.planTrajectory(robotState, worldState, q_des);

                    planning_time = info.time/1000
                    if ~isempty(trajectory)
                        k_opt = info.parameters;
                        if planning_time > P.t_plan
