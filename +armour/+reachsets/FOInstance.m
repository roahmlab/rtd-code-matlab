            ...
            for i = 1:self.jrsInstance.n_t
                for j = 1:self.robotInfo.params.pz_nominal.num_bodies
                    for o = 1:length(worldState.obstacles) % for each obstacle
                        ...
                        [A_obs, b_obs] = armour.legacy.pz_roahm.polytope_PH(O_buf, self.obs_frs_combs); % get polytope form
                        ...
                        % constraint PZ:
                        FO_tmp = armour.legacy.pz_roahm.polyZonotope_ROAHM(self.FO{i, 1}{j, 1}.c, self.FO{i, 1}{j, 1}.G, [], self.FO{i, 1}{j, 1}.expMat, self.FO{i, 1}{j, 1}.id);
