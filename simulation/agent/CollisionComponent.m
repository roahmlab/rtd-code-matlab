classdef CollisionComponent < handle
    ...
    methods
        function self = CollisionComponent(arm_info,arm_state_component,kinematics_component)
        ...
        function create_collision_check_patch_data(self)
            ...
            self.collision_check_patch_data.faces = cc_faces_cell ;
            self.collision_check_patch_data.vertices = cc_verts_cell ;
        end
        ...
        function out = get_collision_check_volume(self,q)
            ...
            if nargin < 2
                q = self.arm_state.state(self.arm_state.position_indices,1) ;
            end
            
            [R,T] = self.kinematics.get_link_rotations_and_translations(q) ;
            
            F_cell = self.collision_check_patch_data.faces ;
            V_cell = self.collision_check_patch_data.vertices ;
