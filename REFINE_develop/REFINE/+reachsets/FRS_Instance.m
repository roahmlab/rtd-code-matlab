        ...
        function nlconFunction = genNLConstraint(self, worldState)%genNl constraint called trajopt
            ...
            if (iscell(vehrs))
                for each_zono = 1:num_zonotopes
                    ...
                    Pb_mat = Pb;
                   nlconFunction = @(k) self.eval_constraints(k,PA_mat,Pb_mat);

                    
                end
                self.constraints = nlconFunction;
...
%delta in center-->evaluate constraint
            h = Pa *k- Pb;
