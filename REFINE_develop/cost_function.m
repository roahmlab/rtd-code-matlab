...
clear all;
clc;
% make it to refine objective class
...
classdef cost_function 
    ...
    methods
        function self = cost_function(vehrs,desired_idx,state_u,state_v,state_r,x_des,y_des,h_des)
            ...
            if desired_idx >= 1 && desired_idx <= num_centers/2
                ...
                self.c_x_ = c_x;
                self.c_y_ = c_y;
                self.c_h_ = ToAngle(c_h);
                % generate.c_k_ = slc.center_slc_val_;
                self.c_k_ = 1;
                self.g_x_ = slc.slc_x_;
                self.g_y_ = slc.slc_y_;
                self.g_h_ = slc.slc_h_;
                self.g_k_ = slc.slc_val_;
                self.x_des_ = x_des;
                self.y_des_ = y_des;
                self.h_des_ = h_des;
            else
                disp('Desired index out of range');
            end    
    ...
    methods
        ...
        function ret_val = ComputeCosts(gen_cost,k,k_min,k_max,eval_opt)
            ...
            ret_val.cost_k = weight_h * sqrt((h_des - c_h + (g_h * (c_k - k))/g_k).^2 + ...
                           weight_xy * (x_des - c_x + (g_x * (c_k - k))/g_k).^2 + ...
                           (y_des - c_y + (g_y * (c_k - k))/g_k).^2);
            ...
            ret_val.hess_k_ = (weight_xy * ((2.0 * gx2) / gk2 + (2.0 * gy2) / gk2)) / (2.0 * sqrt((x_des - c_x + (g_x * (c_k - k)) / g_k).^2 + (y_des - c_y + (g_y * (c_k - k)) / g_k).^2)) - ...
                          (weight_xy * ((2.0 * g_x * (x_des - c_x + (g_x * (c_k - k)) / g_k)) / g_k + (2.0 * g_y * (y_des - c_y + (g_y * (c_k - k)) / g_k)) / g_k).^2) / (4 * ((x_des - c_x + (g_x * (c_k - k)) / g_k).^2 + (y_des - c_y + (g_y * (c_k - k)) / g_k).^2).^(3/2)) - ...
                          (gh2 * weight_h) / (gk2 * sqrt((h_des - c_h + (g_h * (c_k - k)) / g_k).^2)) / ((h_des - c_h + (g_h * (c_k - k)) / g_k).^2).^(3/2);
