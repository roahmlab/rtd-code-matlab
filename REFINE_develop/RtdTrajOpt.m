    ...
    methods
        ...
        function [trajectory, cost, info] = solveTrajOpt(  ...
            ...
            for rs_name=fieldnames(self.reachableSets).'
                ...
                % disp(rs_name)
                % disp(self.reachableSets.rs_name)
                %check for rs_name
                %check for vehrs format storing something is not right
                % disp(self.reachableSets)
                % disp(rs_name{1})
                ...
                for idx=1:length(rs_struct)
                    ...
                    % disp('rs_struct')
                    % disp(rs_struct{idx})
                    id = rs_struct{idx}.id;%cell array bracket
                    rsInstances_arr(id).id = id;
                    rsInstances_arr(id).rs.(rs_name{1}) = rs_struct{idx}.rs;%cell arrray bracket
...
%                     rs_args = namedargs2cell(rsAdditionalArgs.(rs_name{1}));
%                 end
%                 % disp(self.reachableSets.classname)
%                 % frs = self.reachableSets.classname;
%                 frsClassName = self.reachableSets.classname;
%                 frsInstance = FRS_loader_speed_change('converted_Au_frs.h5',25);
%                 rs_struct = frsInstance.getReachableSet(robotState, rs_args{:}, ignore_cache=false);
%                 disp('display of rs_struct')
%                 disp(rs_struct())
%                 % expand struct into id's
%             %     for idx=1:length(rs_struct)
%             %         id = rs_struct(idx).id;
...
%             end
%             % reachableSets = struct;
%             % reachableSets.frs = FrsLoaderObject(blah);
% 
%             % Discard empties
%             mask = [rsInstances_arr.num_instances] > 0;
%             rsInstances_arr = rsInstances_arr(mask);
...
%             %                             rsInstances_cell, ...
%             %                             UniformOutput = false);
%             nlconCallbacks = cell(1, numel(rsInstances_arr));
%             for j = 1:numel(rsInstances_arr)
%                 rsInstances = FRS_Instance_speed_change(rsInstances_arr(j).rs);
%                 nlconCallbacks{j} = @(k) rsInstances.genNLConstraint(worldState, k);
%                 rsInstances_cell = {rsInstances};
% 
% 
%                 % Validate rs sizes
%                 self.vdisp("Validating sizes", 'DEBUG')
%                 % num_parameters = cellfun(@(rs)rs.num_parameters, rsInstances_cell);
%                 num_parameters = cellfun(@(rs)frsInstance.num_parameters, rsInstances_cell);
%                 if ~all(num_parameters == num_parameters(1))
%                     error("Reachable set parameter sizes don't match!")
%                 end
%                 num_parameters = num_parameters(1);
...
%                 % Create the objective
%                 % objectiveCallback = self.objective.genObjective(robotState, ...
%                 %     waypoint, rsInstances); %ORIGINAL
% 
%                  objectiveCallback = self.objective.genObjective(); 
% 
%                 % If initialGuess is none, or invalid, make a zero
%                 try
%                     guess = initialGuess.trajectoryParams;
